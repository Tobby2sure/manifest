...
export function FeedClient({ intents: initialIntents, total, initialFilters }: FeedClientProps) {
  ...
  return (
    <>
      {/* Search + Sort + Filter — full-width row matching card width */}
      <div className="flex flex-row items-center gap-2 mb-4 lg:hidden">
        ...
        <select
          ...
          className="h-11 shrink-0 rounded-xl border border-white/8 bg-white/3 backdrop-blur-sm px-3.5 text-sm text-text-heading outline-none cursor-pointer hover:border-white/12 transition-all duration-200"
        >
        ...
        <Button
          ...
          className={`shrink-0 text-text-body lg:hidden cursor-pointer h-11 w-11 rounded-xl p-0 ${showSidebar ? "bg-white/6 text-violet-400" : "hover:bg-white/4"}`}
        >
